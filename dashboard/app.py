...
def discover_pairs():
    ...
    src_files = {f.stem.upper(): f for f in SOURCE_DIR.iterdir() if f.suffix.lower() in SUPPORTED_EXT}
    tgt_files = {f.stem.upper(): f for f in TARGET_DIR.iterdir() if f.suffix.lower() in SUPPORTED_EXT}
    pairs = []
    for name in sorted(set(src_files) | set(tgt_files)):
        ...
        hp = name in src_files and name in tgt_files
        mt = max(Path(sp).stat().st_mtime, Path(tp).stat().st_mtime) if hp else None
        pairs.append({"name": name, "source_path": sp, "target_path": tp,
                      "has_pair": hp, "mtime": mt,
...
def run_validation(name, source_path, target_path):
    ...
    result_dict = {
        "name": name, "status": result.overall_status,
        "source_file": Path(source_path).name, "target_file": Path(target_path).name,
        ...
        "fields_passed":   ss["fields_passed"],  "fields_failed": ss["fields_failed"],
    ...
    try:    generate_excel_report(result_dict, str(excel_path))
...
def scan_and_validate_all():
    scan_status["scanning"] = True; scan_status["error"] = None
    try:
        for pair in discover_pairs():
            ...
            if not pair["has_pair"]:
                ...
                if prev.get("state") != "unmatched":
                    ...
                    log_event(f"{name}: found in {side} only — waiting for {other} file", "warn")
            ...
            if not existing:
                log_event(f"{name}: new file pair — {pair['source_file']} + {pair['target_file']}", "info")
        ...
        scan_status["error"] = str(e); log_event(f"Scan error: {e}", "error")
    finally:
...
def background_watcher(interval=5):
    while True:
        scan_and_validate_all(); time.sleep(interval)
...
def api_status():
    pairs = discover_pairs(); cfg = load_config()
    return jsonify({
        "last_scan": scan_status["last_scan"], "scanning": scan_status["scanning"],
        "error": scan_status["error"],
        "source_dir": str(get_dirs()[0]), "target_dir": str(get_dirs()[1]),
...
def api_results(): return jsonify(list(results_store.values()))
...
def api_activity(): return jsonify(list(reversed(activity_log)))

@app.route("/api/upload/source", methods=["POST"])
def upload_source(): return _handle_upload(request, get_dirs()[0], "source")

@app.route("/api/upload/target", methods=["POST"])
def upload_target(): return _handle_upload(request, get_dirs()[1], "target")
...
def upload_labels():
    ...
    for n in file_states:
        if file_states[n].get("state") == "done": file_states[n]["state"] = "changed"
    threading.Thread(target=scan_and_validate_all, daemon=True).start()
...
def _handle_upload(req, dest_dir, side):
    ...
    if saved: threading.Thread(target=scan_and_validate_all, daemon=True).start()
    return jsonify({"ok": True, "saved": saved})

@app.route("/api/config", methods=["GET"])
def api_get_config():
    cfg = load_config()
    # Return available fields from any already-validated result
    available = []
    if results_store:
        first = next(iter(results_store.values()))
        available = [{"field": fr["field"], "label": fr.get("field_label", fr["field"])}
...
def api_set_config():
    data = request.get_json(force=True); cfg = load_config(); changed = False
    for key in ("source_dir", "target_dir"):
        if key in data and data[key].strip():
            if str(Path(data[key].strip())) != cfg.get(key):
                cfg[key] = str(Path(data[key].strip())); changed = True
    ...
    if "selected_fields" in data:
        ...
        if sel != cfg.get("selected_fields", []):
            ...
            log_event(f"Field selection updated: {len(sel)} fields" if sel else "Field selection: all fields", "info")
    if changed:
        save_config(cfg); results_store.clear()
        for n in file_states:
            if file_states[n].get("state") == "done": file_states[n]["state"] = "changed"
        threading.Thread(target=scan_and_validate_all, daemon=True).start()
...
def api_labels_sample():
    ...
    lines = ["FIELD_NAME,FRIENDLY_LABEL"] + [f"{k},{v}" for k, v in list(SAP_FIELD_LABELS.items())[:20]]
...
def api_download_file(filename):
    ...
    if not path.exists() or not filename.endswith(".xlsx"): return jsonify({"error": "Not found"}), 404
